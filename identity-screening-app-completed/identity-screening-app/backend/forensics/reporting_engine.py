...
def create_cybercrime_case_payload(
    ...
    """
    Constructs a complete structured JSON dossier for cybercrime reporting.
    ...
    return {
        "case_id": cid,
        "report_title": "AI FORENSIC IDENTITY SCREENING & CYBERCRIME INCIDENT REPORT",
        ...
        "submission_portal": {
            ...
            "submission_notice": (
                ...
                "No automated submission to police databases has occurred."
            )
        },
        "evidence_file": {
        ...
        "extracted_ocr_data": {
            "is_readable": ocr_data.get("is_readable", False),
            "confidence_score": ocr_data.get("overall_confidence", 0.0),
            "document_type": ocr_data.get("fields", {}).get("document_type", "UNKNOWN"),
            "name": ocr_data.get("fields", {}).get("name"),
            "dob": ocr_data.get("fields", {}).get("dob"),
            "doc_number": ocr_data.get("fields", {}).get("doc_number"),
            "expiry_date": ocr_data.get("fields", {}).get("expiry_date"),
            "address": ocr_data.get("fields", {}).get("address"),
            "raw_text_snippet": ocr_data.get("raw_text", "")[:300]
        },
        "mrz_validation_findings": {
            "mrz_detected": mrz_data.get("mrz_detected", False),
            "is_applicable": mrz_data.get("is_applicable", False),
            "status": mrz_data.get("status", "NOT_APPLICABLE"),
            "format": mrz_data.get("format", "NONE"),
            "checksums_passed": mrz_data.get("all_checksums_valid", True),
            "checksum_details": mrz_data.get("checksums", []),
            "mrz_parsed_fields": mrz_data.get("parsed_fields", {})
        },
        "barcode_qr_findings": {
            "detected": barcode_data.get("detected", False),
            "status": barcode_data.get("status", "NOT_FOUND"),
            "format": barcode_data.get("primary_format", "NONE"),
            "cross_match_verdict": barcode_data.get("status_detail"),
            "comparisons": barcode_data.get("comparison_results", []),
            "is_genuine_proof_warning": barcode_data.get("is_genuine_proof_warning")
        },
        "analyst_notes": analyst_notes or "Case escalated for potential synthetic identity / document alteration review."
    }
...
def generate_pdf_case_report(case_payload: Dict[str, Any], output_path: Optional[str] = None) -> bytes:
    ...
    header_table_data = [
        [
            Paragraph("<b>FORENSIC IDENTITY SCREENING & CYBERCRIME CASE REPORT</b>", title_style),
        ],
        [
    ...
    # 2. Metadata Grid
    verdict_color = colors.HexColor('#fee2e2') if "HIGH" in final_verdict else colors.HexColor('#fef3c7')
    verdict_text_color = "#b91c1c" if "HIGH" in final_verdict else "#b45309"
    ...
    meta_table_data = [
        ...
        [
            ...
            Paragraph(f"<font size='6.5' face='Courier'>{case_payload.get('evidence_file', {}).get('sha256_digest', 'N/A')}</font>", body_style),
    ...
    portal_text = (
        ...
        f"<font color='#64748b' size='7.5'>{portal_info.get('submission_notice')}</font>"
    )
    ...
    # 4. Multi-Modal Verification Summary (OCR, MRZ, Barcode, Forensics)
    ...
    findings_table_data = [
        ...
        [
            ...
            Paragraph(f"Conf: {ocr.get('confidence_score', 0):.1f}% | Name: {ocr.get('name') or 'N/A'} | ID: {ocr.get('doc_number') or 'N/A'}", body_style),
        ],
        [
    ...
    findings_table = Table(findings_table_data, colWidths=[120, 110, 80, 230])
    ...
    ident_data = [
        ...
        [
            ...
            Paragraph(str(ocr.get("doc_number") or "NOT DETECTED"), body_style),
        ...
        [
            ...
            Paragraph(str(ocr.get("expiry_date") or "N/A"), body_style),
